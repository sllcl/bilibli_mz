  ...
  <div>
    <div class="h">
      <div class="wrapper">
        <div
          class="h-inner"
          :style="{ backgroundImage: 'url(' + userInfo.bannerImg + ')' }"
        >
          ...
          <div class="h-user">
            <div class="h-info clearfix">
              <div class="h-avatar">
                ...
                <span
                  v-if="userInfo.auth"
                  :title="userInfo.auth.category"
                  class="user-auth clearfix"
                  :class="
                    userInfo.auth.cate === 'personal' ? 'personal-auth' : ''
                  "
                ></span>
              </div>
              <div class="h-basic">
                <div>
                  ...
                  <span
                    id="h-gender"
                    class="icon gender"
                    :class="userInfo.gender === 'male' ? 'male' : 'female'"
                  ></span>
                  <router-link
                    :to="{ path: '/level' }"
                    target="_blank"
                  ...
                  ></router-link>
                  <router-link
                    v-if="userInfo.vipType"
                    :to="{ path: '/accoutbig' }"
                    target="_blank"
                    class="h-vipType"
                    >{{ userInfo.vipType }}</router-link
                  >
                  <router-link
                    v-if="userInfo.fansIcon.haveFansIcon"
                    :to="{ path: userInfo.fansIcon.detail }"
                    target="_blank"
                    class="h-fans-icon"
                  >
                    <span class="h-fans-text">粉</span>
                    <div class="fans-hover-tip">
                      <span class="tip-inner">已开通专属粉丝勋章</span>
                    </div>
                  </router-link>
                </div>
          ...
          <div class="h-action">
            <div
              class="be-dropdown h-f-btn h-unfollow"
              @mouseenter="followHandle(1)"
              @mouseleave="followHandle(2)"
            >
              <div class="h-btn-box">
              ...
              <transition name="zoom">
                <ul
                  v-show="showDropDownFollow"
                  class="be-dropdown-menu"
                  style="left: -10px; transform-origin: center top;"
                >
            ...
            <router-link to="/message" target="_blank" class="h-f-btn h-message"
              >发消息</router-link
            >
            <div
              class="be-dropdown h-add-to-black"
              @mouseenter="moreHandle(1)"
              @mouseleave="moreHandle(2)"
            >
              <div class="be-dropdown-trigger">
                <i
                  title="更多操作"
                  class="bilifont bili-icon_caozuo_xiangyou-copy"
                ></i>
              </div>
              <transition name="zoom">
                <ul
                  v-show="showMore"
                  class="be-dropdown-menu"
                  style="left: -76px; transform-origin: center top;"
                >
                  <li class="be-dropdown-item">加入黑名单</li>
                  <li class="be-dropdown-item">转为悄悄关注</li>
                  <li class="be-dropdown-item" @click="showModal = true">
                    个人信息举报
                  </li>
                </ul>
              </transition>
          ...
          <div class="modal">
              ...
              <div class="report-popup">
                ...
                <ul class="report-popup-list clearfix">
                  <li
                    ...
                    :class="modalData.avatar ? 'checked' : ''"
                    @click="modalData.avatar = !modalData.avatar"
                  >
                  ...
                  <li
                    class="report-popup-item"
                    :class="modalData.nickName ? 'checked' : ''"
                    @click="modalData.nickName = !modalData.nickName"
                  >
                    <em class="report-popup-item-checkbox"></em>
                  ...
                  <li
                    class="report-popup-item"
                    :class="modalData.sign ? 'checked' : ''"
                    @click="modalData.sign = !modalData.sign"
                  >
                    <em class="report-popup-item-checkbox"></em>
            ...
            <div class="modal-footer">
              <button
                ...
                @click="
                  showModal = false
                  modalData = { avatar: false, nickName: false, sign: false }
                "
              >
      ...
      <div class="wrapper">
        <div class="n-inner clearfix">
          <div class="n-tab-links">
            ...
            <router-link
              :key="i"
              v-for="(item, i) in tabLinks"
              :to="{ name: item.component, params: { id: userInfo.id } }"
              class="n-btn"
              :class="item.id === n_activeId ? 'active' : ''"
              :id="item.id"
              @mouseenter.native="cursorMove(i)"
              @click.native="
                n_activeId = item.id
                n_activeStat = ''
              "
          ...
          <div class="n-statistics">
            <router-link
              :to="{ name: 'spaceFollow', params: { id: userInfo.id } }"
              class="n-data n-gz"
              :class="n_activeStat === 'follows' ? 'active' : ''"
              :title="userInfo.follows"
              @click.native="n_activeStat = 'follows'"
            >
            ...
            <router-link
              :to="{ name: 'spaceFans', params: { id: userInfo.id } }"
              class="n-data n-fs"
              :class="n_activeStat === 'fans' ? 'active' : ''"
              :title="userInfo.fans"
              @click.native="n_activeStat = 'fans'"
            >
              <p class="n-data-k">粉丝数</p>
              <p id="n-fs" class="n-data-v space-attention">
                {{ userInfo.fans }}
              </p>
            </router-link>
            <div
              :title="'视频、动态、专栏累计获赞' + userInfo.agrees"
              class="n-data n-bf"
            >
            ...
            <div
              :title="'截止昨天，播放数总计为' + userInfo.playCount"
              class="n-data n-bf"
            >
            ...
            <div
              :title="'截止昨天，阅读数总计为' + userInfo.readCount"
              class="n-data n-bf"
            >
    ...
    window.addEventListener(
      ...
      throttle(function() {
        if (
          ...
          that.showFixedNav === false
        )
          return
        if (
          document.documentElement.scrollTop >= 324 &&
          that.showFixedNav === true
        )
          return
        if (
          document.documentElement.scrollTop < 324 &&
...
<style src="../../assets/css/space.css" scoped></style>
